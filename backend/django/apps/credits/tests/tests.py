...
from apps.credits.models import CreditLedgerEntry, CreditEconomyConfig, CreditRecharge
from apps.credits.services.catalog import CREDIT_ACTIONS, credits_for_amount
from apps.credits.services import complete_recharge, get_or_create_wallet, spend_credits
...
class CreditWalletTests(TestCase):
	...
	def test_recharge_callback_grants_credits_once(self):
		wallet = get_or_create_wallet(user=self.user)
		recharge = CreditRecharge.objects.create(
			...
			provider_reference='ws_CO_test_1',
		)
		first = complete_recharge(provider_reference='ws_CO_test_1', metadata={'result_code': 0})
		second = complete_recharge(provider_reference='ws_CO_test_1')
		wallet.refresh_from_db()
		self.assertEqual(first.id, recharge.id)
		self.assertEqual(second.id, recharge.id)
		self.assertEqual(wallet.balance, 2)
		self.assertEqual(CreditLedgerEntry.objects.filter(action='recharge').count(), 1)

	def test_failed_recharge_grants_no_credits(self):
		wallet = get_or_create_wallet(user=self.user)
		CreditRecharge.objects.create(
			...
			credits=2,
			phone_number=self.user.phone,
			provider_reference='ws_CO_test_failed',
			status=CreditRecharge.Status.FAILED,
		)
		wallet.refresh_from_db()
		self.assertEqual(wallet.balance, 0)
		self.assertFalse(CreditLedgerEntry.objects.filter(action='recharge').exists())
