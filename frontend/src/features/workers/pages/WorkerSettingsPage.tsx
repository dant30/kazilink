  ...
  return (
    <section className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="space-y-6">
        ...
        <FormSection title="Worker notifications" description="Choose the updates that matter to your work search." icon={<Bell className="h-4 w-4" />}>
          <div className="space-y-4 rounded-2xl border border-slate-200 bg-slate-50 p-4"><Switch checked={notifications.shifts} onChange={(value) => setNotifications((current) => ({ ...current, shifts: value }))} label="Shift alerts" description="Receive notifications for newly matched or available shifts." /><Switch checked={notifications.messages} onChange={(value) => setNotifications((current) => ({ ...current, messages: value }))} label="Messages" description="Get updates when employers reach out to you." /><Switch checked={notifications.payments} onChange={(value) => setNotifications((current) => ({ ...current, payments: value }))} label="Payments and payouts" description="Track transaction and payout confirmations." /></div>
        </FormSection>
        <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-center gap-2 text-sm text-slate-600"><Briefcase className="h-4 w-4 text-[#FF6B00]" />Your worker account is active.</div><div className="flex gap-3"><Button variant="outline" type="button">Cancel</Button><Button type="button">Save settings</Button></div></div>
      </div>
    </section>
  )
}
