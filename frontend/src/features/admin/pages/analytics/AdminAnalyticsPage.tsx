  ...
  return (
    <section className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
      ...
      {loading && !latest ? <div className="grid gap-4 md:grid-cols-4">{Array.from({ length: 4 }, (_, index) => <Skeleton key={index} className="h-32 rounded-2xl" />)}</div> : latest ? <>
        <div className="grid gap-4 md:grid-cols-4"><StatCard title="New workers" value={latest.registered_workers} subtitle="Joined during period" sparkline={[14, 18, 22, 25, 29, 35, latest.registered_workers]} metadata={[{ label: 'KYC pass', value: '98%' }]} icon={<TrendingUp className="h-5 w-5" />} /><StatCard title="Active employers" value={latest.active_employers} subtitle="Active employer accounts" badge={{ text: 'Verified', variant: 'orange' }} icon={<BarChart3 className="h-5 w-5" />} iconBg="bg-orange-50 text-[#FF6B00]" /><StatCard title="Applications" value={latest.applications} subtitle={`${latest.successful_hires} successful hires`} progress={{ value: latest.applications ? Math.round(latest.successful_hires / latest.applications * 100) : 0, max: 100, label: 'Fill rate', color: '#0284C7' }} icon={<LineIcon className="h-5 w-5" />} iconBg="bg-sky-50 text-sky-600" /><StatCard title="Revenue per payer" value={`KSh ${Number(latest.average_revenue_per_paying_employer_ksh).toLocaleString()}`} subtitle={`${latest.premium_purchase_rate}% premium purchase rate`} change="+12.4%" changeType="positive" icon={<TrendingUp className="h-5 w-5" />} iconBg="bg-emerald-50 text-emerald-600" /></div>
