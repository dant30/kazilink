...
export function RatingsPage() {
  ...
  return <section className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6 lg:px-8"><PageHeader eyebrow="Trust signals" title="Reviews & ratings" description="Read verified feedback from completed KaziLink hires." actions={<div className="flex items-center gap-3"><Star className="hidden h-10 w-10 fill-orange-300 text-orange-300 sm:block" />{isEmployer && <Button onClick={() => setReviewOpen(true)}>Leave a review</Button>}</div>} />{error && <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm font-semibold text-red-700">{error}</div>}<section className="space-y-4"><div className="flex items-center justify-between gap-3"><div><h2 className="text-xl font-black text-[#0A2540]">Recent reviews</h2><p className="text-sm text-slate-500">{reviews.length} review{reviews.length === 1 ? '' : 's'} available</p></div><Button variant="ghost" size="sm" onClick={() => refresh()} disabled={loading} leftIcon={<RefreshCw className="h-4 w-4" />}>Refresh</Button></div>{loading && !reviews.length ? <p className="rounded-xl bg-slate-50 p-8 text-center text-sm text-slate-500">Loading reviews...</p> : reviews.length ? <>{visibleReviews.map((review) => <ReviewCard key={review.id} review={review} />)}<Pagination page={page} pageSize={pageSize} total={reviews.length} onPageChange={setPage} /></> : <p className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-10 text-center text-sm text-slate-500">No reviews have been submitted yet.</p>}</section><Modal isOpen={reviewOpen} onClose={() => setReviewOpen(false)} title="Leave a review" subtitle="Reviews are available for completed hires." maxWidth="lg"><ReviewForm hires={eligibleHires} onSubmit={submitReview} submitting={submitting} /></Modal></section>
}
