...
export function ReviewForm({ hires, onSubmit, submitting, isWorker = false }: { hires: JobApplication[]; onSubmit: (data: ReviewInput) => Promise<unknown>; submitting?: boolean; isWorker?: boolean }) {
  const [selected, setSelected] = useState<JobApplication | null>(null)
  const [rating, setRating] = useState(5)
  const [comment, setComment] = useState('')
  const [role, setRole] = useState('')
  const submit = async (event: FormEvent) => { event.preventDefault(); if (!selected || (isWorker && selected.employer === undefined)) return; await onSubmit(isWorker ? { target_employer: selected.employer, job: selected.job, rating, comment, role_performed: role } : { target_worker: selected.worker, job: selected.job, rating, comment, role_performed: role }); setSelected(null); setComment(''); setRole('') }
