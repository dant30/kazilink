...
export function PaymentsPage() {
  ...
  const submitPayment = async (data: Parameters<typeof initiatePayment>[0]) => { await initiatePayment(data); setPaymentOpen(false) }
  return <section className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6 lg:px-8"><PageHeader eyebrow="Employer billing" title="Payments" description="Manage M-Pesa payments and review your transaction history." actions={<Button onClick={() => setPaymentOpen(true)} leftIcon={<CreditCard className="h-4 w-4" />}>Make a payment</Button>} />{error && <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm font-semibold text-red-700">{error}</div>}{notice && <div className="rounded-xl border border-green-200 bg-green-50 p-4 text-sm font-semibold text-green-700">{notice}</div>}<section className="rounded-2xl border border-slate-200 bg-slate-50 p-5"><div className="mb-4 flex items-center justify-between gap-3"><div><h2 className="text-lg font-black text-[#0A2540]">Transaction history</h2><p className="text-sm text-slate-500">Payments initiated from your employer account.</p></div><Button variant="ghost" size="sm" onClick={() => refresh()} disabled={loading} leftIcon={<RefreshCw className="h-4 w-4" />}>Refresh</Button></div>{loading && !initialized ? <p className="py-10 text-center text-sm text-slate-500">Loading transactions...</p> : transactions.length ? <><div className="space-y-3">{visibleTransactions.map((transaction) => <TransactionCard key={transaction.id} transaction={transaction} onRefund={refund} refunding={refundingId === transaction.id} />)}</div><Pagination page={page} pageSize={pageSize} total={transactions.length} onPageChange={setPage} className="mt-4" /></> : <p className="rounded-xl border border-dashed border-slate-200 bg-white p-10 text-center text-sm text-slate-500">No transactions yet.</p>}</section><Modal isOpen={paymentOpen} onClose={() => setPaymentOpen(false)} title="Make a payment" subtitle="Complete the details to receive an M-Pesa prompt." maxWidth="lg"><PaymentForm onSubmit={submitPayment} submitting={processing} defaultPhone={user?.phone} /></Modal></section>
}