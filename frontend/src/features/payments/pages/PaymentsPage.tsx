...
import { CreditWalletPanel, PaymentForm, TransactionCard } from '../components'
...
export function PaymentsPage() {
  ...
  if (!isEmployer) {
    return <section className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6 lg:px-8"><PageHeader eyebrow="Kazi Credits" title="Your wallet" description="Recharge credits for premium job access and profile visibility." /><CreditWalletPanel phone={user?.phone} role="worker" /></section>
  }

  const submitPayment = async (data: Parameters<typeof initiatePayment>[0]) => {
    try {
      await initiatePayment(data)
      setPaymentOpen(false)
    } catch (submissionError) {
      toast.error('Payment failed', submissionError instanceof Error ? submissionError.message : 'Unable to complete the payment request.')
    }
  }

  return <section className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
    <PageHeader eyebrow="Employer billing" title="Payments" description="Manage M-Pesa payments, Kazi Credits, and your transaction history." actions={<Button onClick={() => setPaymentOpen(true)} leftIcon={<CreditCard className="h-4 w-4" />}>Make a payment</Button>} />
    <CreditWalletPanel phone={user?.phone} role="employer" />
    ...
    <Modal isOpen={paymentOpen} onClose={() => setPaymentOpen(false)} title="Make a payment" subtitle="Complete the details to receive an M-Pesa prompt." maxWidth="lg"><PaymentForm onSubmit={submitPayment} submitting={processing} defaultPhone={user?.phone} /></Modal>
  </section>
}
