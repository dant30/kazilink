...
import { CreditRechargeForm, CreditWalletPanel, TransactionCard } from '../components'
...
export function PaymentsPage() {
  ...
  if (!isEmployer) {
    return <section className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6 lg:px-8"><PageHeader eyebrow="Kazi Credits" title={`Credit balance: ${creditBalance ?? '...'} Kazi Credits`} description="Recharge credits for premium job access and profile visibility." actions={<Button onClick={() => setPaymentOpen(true)} leftIcon={<CreditCard className="h-4 w-4" />}>Recharge</Button>} /><CreditWalletPanel role="worker" refreshToken={walletRefreshToken} onBalanceChange={setCreditBalance} /><Modal isOpen={paymentOpen} onClose={() => setPaymentOpen(false)} title="Recharge Kazi Credits" subtitle="Complete the M-Pesa prompt to add credits to your wallet." maxWidth="md"><CreditRechargeForm defaultPhone={user?.phone} onComplete={() => { setWalletRefreshToken((value) => value + 1); setPaymentOpen(false) }} /></Modal></section>
  }

  return <section className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
    <PageHeader eyebrow="Kazi Credits" title={`Credit balance: ${creditBalance ?? '...'} Kazi Credits`} description="Recharge credits for premium hiring actions and job visibility." actions={<Button onClick={() => setPaymentOpen(true)} leftIcon={<CreditCard className="h-4 w-4" />}>Recharge</Button>} />
    ...
    <Modal isOpen={paymentOpen} onClose={() => setPaymentOpen(false)} title="Recharge Kazi Credits" subtitle="Complete the M-Pesa prompt to add credits to your wallet." maxWidth="md"><CreditRechargeForm defaultPhone={user?.phone} onComplete={() => { setWalletRefreshToken((value) => value + 1); setPaymentOpen(false) }} /></Modal>
  </section>
}
