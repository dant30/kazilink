    ...
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="divide-y divide-slate-100 rounded-lg border border-slate-100">
          {wallet?.ledger.slice(0, 10).map((entry) => <div key={entry.id} className="flex items-start gap-3 px-3 py-3 text-sm"><span className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${entry.amount > 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-orange-50 text-[#FF6B00]'}`}>{entry.amount > 0 ? <ArrowDownLeft className="h-3.5 w-3.5" /> : <ArrowUpRight className="h-3.5 w-3.5" />}</span><span className="min-w-0 flex-1"><span className="block text-slate-600">{entry.action.replace(/_/g, ' ')}</span><span className="mt-0.5 block text-[11px] text-slate-400">Balance after: {entry.balance_after} · {new Date(entry.created_at).toLocaleString()}</span></span><strong className={entry.amount > 0 ? 'text-emerald-600' : 'text-slate-900'}>{entry.amount > 0 ? '+' : ''}{entry.amount}</strong></div>)}
