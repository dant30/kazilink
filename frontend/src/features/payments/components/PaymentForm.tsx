...
export function PaymentForm({ onSubmit, submitting, defaultPhone }: { onSubmit: (data: PaymentInitiateInput) => Promise<unknown>; submitting?: boolean; defaultPhone?: string }) {
  ...
  return <form id="payment-form" onSubmit={submit}><FormSection title="Make a payment" description="A secure M-Pesa prompt will be sent to your phone." icon={<CreditCard className="h-4 w-4" />} divider={false}><FormField label="Payment purpose" required><select value={form.transaction_type} onChange={(event) => setForm({ ...form, transaction_type: event.target.value })} className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm">{types.map((type) => <option key={type.value} value={type.value}>{type.label}</option>)}</select></FormField><FormField label="Amount (KSh)" required><Input required type="number" min={1} value={form.amount_ksh || ''} onChange={(event) => setForm({ ...form, amount_ksh: Number(event.target.value) })} /></FormField><FormField label="M-Pesa phone number" required><Input required value={form.phone_number || ''} onChange={(event) => setForm({ ...form, phone_number: event.target.value })} placeholder="2547XXXXXXXX" /></FormField></FormSection><FormActions formId="payment-form" submitLabel="Send payment prompt" loading={submitting} disabled={form.amount_ksh < 1} /></form>
}
