...
export function CreditRechargeForm({ defaultPhone, onComplete, onCancel }: { defaultPhone?: string; onComplete: () => void; onCancel?: () => void }) {
  ...
  return <form onSubmit={submit} className="space-y-4">
    ...
    {status === 'idle' && <>
      <div className="flex items-start gap-3 rounded-xl border border-sky-100 bg-sky-50 p-3 text-sm text-sky-900"><CreditCard className="mt-0.5 h-4 w-4 shrink-0" /><p><strong>{amount > 0 ? Math.floor(amount / rate) : 0} Kazi Credits</strong> will be added after successful M-Pesa confirmation.</p></div>
