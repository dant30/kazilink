...
export const paymentStore = {
	...
	async initiate(data: PaymentInitiateInput) { state = { ...state, processing: true, error: null, notice: null }; notify(); try { const result = await paymentServices.initiate(data); state = { ...state, transactions: [result.transaction, ...state.transactions], processing: false, notice: 'Payment prompt sent. Complete it on your phone.' }; notify(); return result } catch (error) { state = { ...state, processing: false, error: null }; notify(); throw error } },
