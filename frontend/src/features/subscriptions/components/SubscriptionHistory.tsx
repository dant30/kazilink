...
export function SubscriptionHistory({ subscriptions, processing, onCancel }: { subscriptions: Subscription[]; processing: boolean; onCancel: (id: number) => Promise<unknown> }) {
  if (!subscriptions.length) return <EmptyState title="No subscription history yet" description="Your subscription records will appear here." icon={<CalendarDays className="h-8 w-8" />} size="sm" />
