...
export function RegisterPage() {
  ...
  return <AuthPanel eyebrow="Join KaziLink" title="Create your account" subtitle="Start hiring faster or find work faster with a trusted Kenyan recruitment community.">
    <FormSection title="Your profile" description="Set up your worker or employer account and verify the details that matter most.">
      ...
      <div className="fixed inset-x-0 bottom-16 z-30 border-t border-slate-200 bg-white/95 px-3 py-2.5 shadow-[0_-8px_24px_rgba(15,23,42,0.08)] backdrop-blur-sm sm:static sm:border-0 sm:bg-transparent sm:px-0 sm:py-0 sm:pt-4 sm:shadow-none"><div className="mx-auto max-w-xl space-y-2 sm:max-w-none sm:space-y-3"><button form="register-form" type="submit" disabled={saving || !termsAccepted || !privacyAccepted} className="flex min-h-[48px] w-full items-center justify-center gap-2 rounded-xl bg-[#0A2540] px-4 py-3 text-sm font-black text-white shadow-md transition hover:bg-[#FF6B00] disabled:opacity-50">{saving ? 'Creating account...' : 'Create KaziLink Account'}</button><div className="text-center text-xs text-slate-600 sm:text-sm">Already registered? <Link className="font-bold text-[#0A2540] hover:text-[#FF6B00]" to="/login">Sign in to your account</Link></div></div></div>
    </FormSection>
  </AuthPanel>
}
