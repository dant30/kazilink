...
export function SupportTicketList({ tickets, onClose, closingId }: { tickets: SupportTicket[]; onClose: (id: number) => Promise<unknown>; closingId?: number | null }) {
  if (!tickets.length) return <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-10 text-center"><CircleHelp className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-3 text-sm font-semibold text-slate-600">No support tickets yet.</p></div>
