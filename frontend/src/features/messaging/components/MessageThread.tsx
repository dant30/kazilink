...
export function MessageThread({ messages, currentUserId, loading }: { messages: Message[]; currentUserId?: number; loading?: boolean }) {
  ...
  if (loading) return <div className="flex flex-1 items-center justify-center text-sm text-slate-500">Loading messages...</div>
  if (!messages.length) return <div className="flex flex-1 items-center justify-center px-6 text-center text-sm text-slate-500">No messages yet. Start the conversation below.</div>
